...
- **EFS ファイルシステム作成（添付画面ベースのステップ）**  
  ...
  3. ステップ2「ネットワークアクセス」:  
     ...
     - マウントターゲットの入力例（添付画面と同じ配置で、AZごとに1行ずつ追加）:  
       - アベイラビリティーゾーン: `ap-northeast-1a`（2つ目を作るときは `ap-northeast-1c`）  
       - サブネットID: プライベートサブネットのID（例: `subnet-0a2cfa9b...` = `in-apne1a`、2行目は `in-apne1c`）  
       - IP アドレスタイプ: `IPv4 のみ`  
       - IPv4 アドレス: `オプション` のまま空欄（自動割り当て）  
       - セキュリティグループ: `sg-efs-palworld` を選択（2049/tcp を受ける）  
     - 「マウントターゲットを追加」で 1c 側の行を追加し、両AZぶん作成する。  
