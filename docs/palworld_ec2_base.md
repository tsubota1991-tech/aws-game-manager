...
- **VPC/サブネットの作成**  
  ...
  2. そのままか、左メニュー「サブネット」→「サブネットを作成」で次を入力して 2AZ 分作成:  
     ...
     - IPv4 サブネット CIDR: 本番 `10.0.0.0/26`（apne1a）、`10.0.0.64/26`（apne1c）／検証なら `/28` に縮小可  
       - 理由: `/26` を使うと残りのアドレス帯にパブリックサブネット（例: `10.0.0.192/28`）を置けるため、重複エラーを回避できる  
...
- **インターネット/NAT の作り方（コンソール手順）**:  
  ...
  2. 左メニュー「サブネット」→ パブリック用に新規サブネット（例: `10.0.0.192/28`, AZ=apne1a, タグ `Name=pub-apne1a`）。`/26` で確保したプライベートと重ならない CIDR を選ぶ。  
