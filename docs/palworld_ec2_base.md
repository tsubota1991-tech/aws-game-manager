...
- **セキュリティグループ (SG) を画面の入力項目に紐付けて作成**  
  ...
  7. （ここまでで作成した 2 つの SG を、後続の画面で選び間違えないようにするための補足）**どの画面でどの SG を選ぶか**  
     - Interface VPC エンドポイント（順番どおりにクリックして入力）  
       - 左メニュー「エンドポイント」→「エンドポイントを作成」→ **タイプ=AWS のサービス** を選択（添付スクリーンショットの画面）→ サービス名で `ssm` などを検索・選択。  
       - 「VPC」プルダウン=`pal-spot-vpc` → 「サブネット」チェックボックス=プライベート 2 つ（`in-apne1a`/`in-apne1c`） → **「セキュリティグループ」プルダウン=`sg-ec2-palworld-spot`** を選択（443 を許可しているため Interface エンドポイントの https 通信に利用）。  
       - そのまま「エンドポイントを作成」。  
     - EFS マウントターゲット（EFS ウィザードの流れに続けて入力）  
       - 左メニュー「EFS」→ 対象ファイルシステムをクリック → 右上「ネットワーク」タブ。既存なら「マウントターゲットを編集」、新規なら作成ウィザードの「ネットワーク」セクション。  
       - 「VPC」=`pal-spot-vpc` → 「サブネット」=プライベート 2 つ（`in-apne1a`/`in-apne1c`） → **「セキュリティグループ」チェックボックス=`sg-efs-palworld`** を選択（2049/tcp を受けるため）。  
       - 保存/作成を押す。  
     - EC2 起動/Launch Template（インスタンス起動ウィザードと同じ配置）  
       - 左メニュー「起動テンプレート」→「起動テンプレートの作成」。  
       - 画面「ネットワーキング」セクションで「VPC」=`pal-spot-vpc` → 「サブネット」=`in-apne1a`（ASG で複数 AZ を使う場合は LT で「サブネットなし」を選び、ASG 側で AZ/サブネットを指定する） → **「セキュリティグループ」チェックボックス=`sg-ec2-palworld-spot`** を選択（8211/udp と 22/tcp を開けるため）。  
       - テンプレートを作成（またはインスタンス起動時に同じ欄で SG を選択）。
