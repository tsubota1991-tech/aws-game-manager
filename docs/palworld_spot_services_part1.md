...
| アベイラビリティーゾーン | `ap-northeast-1a` を明示選択（指定なしでも可） | `ap-northeast-1c` を明示選択 |
| IPv4 VPC CIDR ブロック | `10.0.0.0/24`（VPC で指定したものをプルダウンから選択） | `10.0.0.0/24` |
| IPv4 サブネット CIDR ブロック | 検証: `10.0.0.0/28` / 本番: `10.0.0.0/26` | 検証: `10.0.0.64/28` / 本番: `10.0.0.64/26` |
| タグ | `Name=in-apne1a`, `Project=palworld`, `Env=prod` | `Name=in-apne1c`, `Project=palworld`, `Env=prod` |

- 以降の ASG/マウントターゲットは上記 2 つのプライベートサブネットを指定する。
...
## 2. ネットワークと EFS
- **サブネット例**: `10.0.0.0/24` VPC 内にプライベートサブネットを 2 つ作成（例: `10.0.0.0/26` in apne1a, `10.0.0.64/26` in apne1c）。パブリックサブネットは空いている帯域に `10.0.0.128/27` など非重複の CIDR を割り当てる（`/25` のままでは重複するので必ず `/26` 以下に分割する）。ASG は両方に配置し、EFS マウントターゲットも各 AZ に配置。
- **セキュリティグループ**: 
  - EC2 用 SG (例: `sg-ec2-palworld-spot`): `8211/udp`（0.0.0.0/0 または必要クライアント）、`22/tcp`（管理元 IP のみ）をインバウンド許可。アウトバウンドは EFS SG に対して `2049/tcp` を許可。
  - EFS 用 SG (例: `sg-efs-palworld`): EC2 SG からの `2049/tcp` をインバウンド許可。アウトバウンドはデフォルト許可。
- **EFS マウントターゲット**: apne1a/apne1c の各プライベートサブネットを選択し、EFS SG をアタッチ。バックアップを有効化する場合は作成時に「自動バックアップ」を ON。
...
- **AMI**: ①で作成したベース AMI
- **インスタンスタイプ**: **起動テンプレートでは 1 つだけ指定**する（例: `c6i.large`）。複数候補は ASG の「インスタンスのオーバーライド」で扱うため、アプリ側のシステム設定にも候補リストを登録しておく（後述）。
- **キーペア**: 管理用を指定
...
## 6. 本システム側で管理するインスタンスタイプ候補
- 管理画面の **System Settings** (`/admin/system-settings`) に「オートスケーリング用インスタンスタイプ」セクションを追加済み。
- ここで複数のインスタンスタイプを選択・入力して保存すると、ASG のオーバーライド候補として運用チームが参照できるリストを DB に保持できる。
- 候補リストはカンマ区切り入力も可能。重複は自動除去される。
- ASG 作成時はこのリストを見ながら `MixedInstancesPolicy > Overrides` に同じ順序で登録することで、起動テンプレートが単一タイプしか持てない制約を補完する。
