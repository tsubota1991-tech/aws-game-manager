...
## 2. ネットワークと EFS
- **サブネット例**: `10.0.0.0/24` VPC 内にプライベートサブネットを 2 つ作成（例: `10.0.0.0/26` in apne1a, `10.0.0.64/26` in apne1c）。残りのアドレス帯にパブリックサブネット（例: `10.0.0.192/28`）を置けるため、重複エラーを避けられる。ASG は両方に配置し、EFS マウントターゲットも各 AZ に配置。
- **セキュリティグループ**: 
