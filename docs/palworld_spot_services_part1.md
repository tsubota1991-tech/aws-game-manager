...
| --- | --- |
| サブネット名 | `in-apne1a` |
| アベイラビリティーゾーン | `ap-northeast-1a`（または指定なしで自動） |
| IPv4 VPC CIDR ブロック | `10.0.0.0/24` |
| IPv4 サブネット CIDR ブロック | 検証最小構成: `10.0.0.0/28` / 本番推奨: `10.0.0.0/25` |
| タグ | `Name=in-apne1a`, `Project=palworld`, `Env=prod` |

- 2AZ 目は `in-apne1c`、CIDR を `10.0.0.128/28`（検証）または `10.0.0.128/25`（本番）で作成。
- 以降の ASG/マウントターゲットはこの 2 つのプライベートサブネットを指定する。
