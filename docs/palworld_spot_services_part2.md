...
- **Mixed Instances Policy**:
  ...
  - **Overrides**: 起動テンプレート側ではインスタンスタイプを 1 つしか持てないため、ここに複数候補を並べる。管理画面の System Settings で保存した「オートスケーリング用インスタンスタイプ」リストをそのまま流用すると、運用中の候補と整合する。
...
- **オートスケーリング用インスタンスタイプ**: System Settings の複数選択/カンマ入力欄で ASG で使う候補リストを保存しておく。運用チームはこのリストを見ながら ASG の Override を更新できる。
